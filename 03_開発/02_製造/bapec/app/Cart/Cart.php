...
use Carbon\Carbon;
use Closure;
use App\Cart\Calculation\CartPriceCalculator;
...
class Cart
{
    ...
    const CART_ATTR = 'cart.attribure';
    ...
    public function addCartItem($item, $keepDiscount = false, $keepTax = false, $dispatchEvent = true)
    {
        ...
        // calc
        $content = $this->calculateContent($content);
    ...
    public function totalFloat()
    {
        return $this->getContent()->reduce(function ($total, CartItem $cartItem) {
            ...
            return $total + $cartItem->subtotalTaxIncluded;
        }, 0);
    }
    ...
    public function initialFloat()
    {
        return $this->getContent()->reduce(function ($initial, CartItem $cartItem) {
            return $initial + ($cartItem->qty * $cartItem->unitPrice);
        }, 0);
    }
    ...
    private function initialize()
    {
        ...
        if (!$this->session->has(self::CART_ATTR)) {
            $this->initializeCartAttribute();
        } else {
            $cartAttr = $this->session->get(self::CART_ATTR);
            if ($cartAttr->getCreatedAt()) {
                if ($cartAttr->getCreatedAt()->lt(Carbon::today())) {
                    $this->initializeCartAttribute();
                }
            } else {
                $this->initializeCartAttribute();
     ...
     * カートの基本属性を構築する
     *
     * @return CartAttribute カート属性
     */
    public function initializeCartAttribute()
    {
        $cartAttr = new CartAttribute();

        // ショップマスタから端数処理方式を取得する
        $shop = Shop::find(1);
        $cartAttr->setTaxRoundingType($shop->tax_rounding_type);
        $cartAttr->setDiscountRoundingType($shop->discount_rounding_type);

        // 顧客ランクから割引率（額）を取得する
        if (\Auth::check()) {
            $customerRank = \Auth::user()->customerRank->first();
            if ($customerRank) {
                $cartAttr->setDiscountPrice($customerRank->discount_price);
                $cartAttr->setDiscountRate($customerRank->discount_rate);
            }
        }

        $cartAttr->setCreatedAt(Carbon::today());

        $this->session->put(self::CART_ATTR, $cartAttr);
     ...
     * 指定されたカート明細の料金計算を行う
     *
     * @param Collection $content カート明細
     * @return Collection カート明細
     */
    protected function calculateContent($content)
    {
        $content = $this->getContent();
        $cartPriceCalculator = new CartPriceCalculator($content);
        $calcContent = $cartPriceCalculator->calculate();
        return $calcContent;
    }
    ...
    {
        $content = $this->getContent();
        $calcContent = $this->calculateContent($content);
        $this->session->put($this->instance, $calcContent);
    }
    ...
    {
        if ($this->session->has(self::CART_ATTR)) {
            return $this->session->get(self::CART_ATTR);
        }

        return $this->initializeCartAttribute();
    }
