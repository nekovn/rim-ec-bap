...
create table `ships` (
  ...
  , `client_zip` varchar(7) comment '依頼主・郵便番号'
  ...
  , `ship_zip` varchar(7) comment '出荷先・郵便番号'
...
create table `order_deliveries` (
  ...
  , `delivery_zip` varchar(7) comment '配送先・郵便番号'
...
--* RestoreFromTempTable
create table `split_deliveries` (
  `id` serial not null auto_increment comment 'ID'
  , `ship_id` bigint unsigned not null comment '出荷ID'
  , `delivery_no` varchar(20) comment '配送伝票番号:お問い合わせ番号'
  , constraint `split_deliveries_PKC` primary key (`id`)
) comment '口割れ' ;
